...
COLUMNA_CLAVE = "accountNumber"   # cámbiala por tu columna
...
def main():
    ...
    datos_B = obtener_datos(DB_B, TABLA_B)
    ...
    for i in datos_B:
        print(i[COLUMNA_CLAVE])
        break
    valores_A = {row[COLUMNA_CLAVE] for row in datos_A}

    # Lista de registros de A que NO están en B
    faltantes = [row[COLUMNA_CLAVE] for row in datos_B if row[COLUMNA_CLAVE] not in valores_A]

    print(f"Registros faltantes encontrados: {len(faltantes)}")


    # Guardar en JSON
    with open("faltantes2.json", "w", encoding="utf-8") as f:
